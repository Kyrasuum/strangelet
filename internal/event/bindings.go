...
func InitBindings() {
	...
	if _, e := os.Stat(filename); e == nil {
		...
		if err != nil {
			app.CurApp.TermMessage("Error reading bindings.json file: " + err.Error())
			return
		}

		err = json5.Unmarshal(input, &parsed)
		if err != nil {
			app.CurApp.TermMessage("Error reading bindings.json:", err.Error())
		}
	}
	...
	for k, v := range parsed {
		...
		case map[string]interface{}:
			bind, ok := Binder[k]
			if !ok || bind == nil {
				app.CurApp.TermMessage(fmt.Sprintf("%s is not a valid pane type", k))
				continue
			}
			for e, a := range val {
				s, ok := a.(string)
				if !ok {
					app.CurApp.TermMessage("Error reading bindings.json: non-string and non-map entry", k)
				} else {
					BindKey(e, s, bind)
				}
			}
