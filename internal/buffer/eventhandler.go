...
func (eh *EventHandler) DoTextEvent(t *TextEvent, useUndo bool) {
	...
	if t.EventType == TextEventInsert {
		...
		t.Deltas[0].End = clamp(cursor.Loc{endX, start.Y + linecount}, eh.buf.LineArray)
	}
	end := t.Deltas[0].End

	for _, c := range eh.cursors {
		move := func(loc cursor.Loc) cursor.Loc {
			if t.EventType == TextEventInsert {
				...
				if loc.Y != end.Y && loc.GreaterThan(end) {
					...
					loc = MoveLA(loc, -DiffLA(start, end, eh.buf.LineArray), eh.buf.LineArray)
				}
				return loc
			}
		}
...
func ExecuteTextEvent(t *TextEvent, buf *SharedBuffer) {
	if t.EventType == TextEventInsert {
		...
		for i, d := range t.Deltas {
			...
			t.Deltas[i].End = cursor.Loc{d.Start.X + util.CharacterCount(d.Text), d.Start.Y}
		}
...
func NewEventHandler(buf *SharedBuffer, cursors []*cursor.Cursor) *EventHandler {
	eh := new(EventHandler)
...
func (eh *EventHandler) ApplyDiff(new string) {
	...
	for _, d := range diff {
		if d.Type == dmp.DiffDelete {
			eh.Remove(loc, MoveLA(loc, util.CharacterCountInString(d.Text), eh.buf.LineArray))
		} else {
			if d.Type == dmp.DiffInsert {
				eh.Insert(loc, d.Text)
			}
			loc = MoveLA(loc, util.CharacterCountInString(d.Text), eh.buf.LineArray)
		}
	}
}

// Insert creates an insert text event and executes it
func (eh *EventHandler) Insert(start cursor.Loc, textStr string) {
	text := []byte(textStr)
	eh.InsertBytes(start, text)
}

// InsertBytes creates an insert text event and executes it
func (eh *EventHandler) InsertBytes(start cursor.Loc, text []byte) {
	if len(text) == 0 {
		return
	}
	...
	e := &TextEvent{
		...
		Deltas:    []Delta{{text, start, cursor.Loc{0, 0}}},
		Time:      time.Now(),
	}
...
func (eh *EventHandler) Remove(start, end cursor.Loc) {
	if start == end {
		return
	}
	start = clamp(start, eh.buf.LineArray)
...
func (eh *EventHandler) Replace(start, end cursor.Loc, replace string) {
	eh.Remove(start, end)
