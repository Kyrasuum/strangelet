...
func (b *Buffer) findDown(r *regexp.Regexp, start, end cursor.Loc) ([2]cursor.Loc, bool) {
	...
	for i := start.Y; i <= end.Y; i++ {
		...
		if match != nil {
			start := cursor.Loc{charpos + util.RunePos(l, match[0]), i}
			end := cursor.Loc{charpos + util.RunePos(l, match[1]), i}
			return [2]cursor.Loc{start, end}, true
		}
	}
	return [2]cursor.Loc{}, false
}

func (b *Buffer) findUp(r *regexp.Regexp, start, end cursor.Loc) ([2]cursor.Loc, bool) {
	lastcn := util.CharacterCount(b.LineBytes(b.LinesNum() - 1))
	if start.Y > b.LinesNum()-1 {
		start.X = lastcn - 1
	}
...
func (b *Buffer) FindNext(s string, start, end, from cursor.Loc, down bool, useRegex bool) ([2]cursor.Loc, bool, error) {
	if s == "" {
		return [2]cursor.Loc{}, false, nil
	}
...
func (b *Buffer) ReplaceRegex(start, end cursor.Loc, search *regexp.Regexp, replace []byte) (int, int) {
	if start.GreaterThan(end) {
		start, end = end, start
	}
	...
	for i := start.Y; i <= end.Y; i++ {
		...
		from := cursor.Loc{charpos, i}
		to := cursor.Loc{charpos + util.CharacterCount(l), i}
