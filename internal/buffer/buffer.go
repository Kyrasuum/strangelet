...
func (b *SharedBuffer) insert(pos cursor.Loc, value []byte) {
	b.isModified = true
...
func (b *SharedBuffer) remove(start, end cursor.Loc) []byte {
	b.isModified = true
	b.HasSuggestions = false
...
func NewBufferFromFile(path string, btype BufType) (*Buffer, error) {
	...
	if os.IsNotExist(err) {
		...
		buf = NewBuffer(file, util.FSize(file), filename, btype)
		if buf == nil {
...
func NewBufferFromString(text, path string, btype BufType) *Buffer {
	return NewBuffer(strings.NewReader(text), int64(len(text)), path, btype)
}
...
func NewBuffer(r io.Reader, size int64, path string, btype BufType) *Buffer {
...
func (b *Buffer) Insert(start cursor.Loc, text string) {
	if !b.Type.Readonly {
...
func (b *Buffer) Remove(start, end cursor.Loc) {
	if !b.Type.Readonly {
...
func (b *Buffer) RuneAt(loc cursor.Loc) rune {
...
func (b *Buffer) WordAt(loc cursor.Loc) []byte {
	...
	for start.X > 0 && util.IsWordChar(b.RuneAt(Move(start, -1, b))) {
		start.X--
	}
...
func (b *Buffer) FindMatchingBrace(braceType [2]rune, start cursor.Loc) (cursor.Loc, bool, bool) {
		...
		for y := start.Y; y < b.LinesNum(); y++ {
			...
			for x := xInit; x < len(l); x++ {
				...
				if r == braceType[0] {
					...
					if i == 0 {
						if startChar == braceType[0] {
							return cursor.Loc{x, y}, false, true
						}
						return cursor.Loc{x, y}, true, true
					}
				}
			}
		}
...
// into a loc
func ParseCursorLocation(cursorPositions []string) (cursor.Loc, error) {
	startpos := cursor.Loc{0, 0}
	var err error
...
func (b *Buffer) SearchMatch(pos cursor.Loc) bool {
